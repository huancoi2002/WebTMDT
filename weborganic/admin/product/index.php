<?php
	...
	$sql = "SELECT product.*, category.name as category_name from product left join category on product.category_id = category.id LIMIT $offset, $itemsPerPage";
	$data = executeResult($sql);
	$totalRows = executeCountTotal('SELECT product.*, category.name as category_name from product left join category on product.category_id = category.id');
?>
<style> 
.my-table {
  width: 100%;
